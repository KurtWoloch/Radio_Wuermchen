...
WEATHER_RESULT = BASE_DIR / "weather_result.txt"
...
MAX_DJ_ATTEMPTS = 3 # CHANGED: Reduced from 5 to 3 to limit history churn on failures
# --- HELPERS ---
def log(msg):
...
def get_weather_forecast():
    """Read cached weather from file, or return None."""
    try:
        if WEATHER_RESULT.exists():
            text = WEATHER_RESULT.read_text(encoding="utf-8").strip()
            return text if text else None
    except Exception as e:
        log(f"Weather read error: {e}")
    return None
            ...
            while retry_count < MAX_DJ_ATTEMPTS and not success:
                ...
                if dj_output:
                    ...
                    if suggested_track and announcement_text:
                        ...
                        # FIX: Compare against history excluding the *last* item, which is the
                        # track just suggested by the DJ Brain in this *very* turn (race condition fix).
                        if history and len(history) > 0:
                            history_to_check = history[:-1]
                        else:
                            history_to_check = history
                            
                        recent_tracks = [entry["track"].lower() for entry in history_to_check if "track" in entry]
                        
                        if suggested_track.lower() in recent_tracks:
                            log(f"DJ Suggestion REJECTED: '{suggested_track}' is in recent history (excluding current suggestion).")
                            found_track = None 
                        else:
