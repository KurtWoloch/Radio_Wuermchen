...
"old_string":"start "Pipe Chain Stream Log" cmd /k "python %PYTHON_SCRIPT% | %FFMPEG_BIN% -i - -c:a copy -f mp3 -content_type audio/mpeg %ICECAST_URL%"","new_string":"start "Pipe Chain Stream Log" cmd /k "python %PYTHON_SCRIPT% | %FFMPEG_BIN% -i - -f mp3 -acodec libmp3lame -content_type audio/mpeg %ICECAST_URL%""

"old_string":"start "Pipe Chain Stream Log" cmd /k "python %PYTHON_SCRIPT% | %FFMPEG_BIN% -i - -f mp3 -acodec libmp3lame -content_type audio/mpeg %ICECAST_URL%"","new_string":"start "Pipe Chain Stream Log" cmd /k "python %PYTHON_SCRIPT% | %FFMPEG_BIN% -i - -f mp3 -acodec libmp3lame -content_type audio/mpeg icecast://source:hackme@localhost:8000/stream"
